module.exports =function(argv){
    ...
    function calcTime(time_log){
        ...
        for(var temp in temp_array){
            ...
            if(log_key!="browser"&&log_key!="phoneid"&&log_key!="logid"&&log_key!="ta_net"
                &&log_key!="taspeed"&&log_key!="ls"&&log_key!='later'&&log_key!="net"){
    ...
    function showResult(){
        ...
        var time_show = time_css+'<table id="mytable"><thead><tr><th>id</th><th>min</th><th>max</th><th>average</th><th>median</th><th>threshold</th><th>count</th><th>result</th></tr></thead><tbody>';
        for(var log_key in time_logs){
            var temp_sort = time_logs[log_key].sort(function(a,b){return a>b?1:-1});
            var median = 0;
            var average = 0;
            var min = temp_sort[0];
            var max = temp_sort[temp_sort.length-1];
            if(temp_sort.length%2==0){
                median = (temp_sort[temp_sort.length/2]+temp_sort[(temp_sort.length/2)-1])/2;
            }
            else{
                median = Math.round(temp_sort[parseInt(temp_sort.length/2)]);
            }
...
// http proxy
    server.on('request', function(request, response) {
        ...
        if (request.url.indexOf("http://111.13.12.43/static/tj.gif")!=-1){
            calcTime(request.url);
        }
//    if(request.url.indexOf("wpotest.com/end")!=-1){
